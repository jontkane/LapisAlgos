...
namespace lapis {
    ...
    CsmPreMade::CsmPreMade(Raster<csm_t>&& raster, CsmMergeFunc mergeFunc) : _raster(std::move(raster)), _mergeFunc(mergeFunc) {}
    ...
    void CsmPreMade::addPointsUnsafe(const std::span<const LasPoint>& points)
    {
        //intentionally empty
    }
